...
try {
  ...
  await msx.type('NEW\r1 CLEAR 200,&HBFFF\r10 IF PEEK(&HC003)=0 THEN 10\r20 M=PEEK(&HC003):POKE &HC003,0:_SCREEN(M):_WAIT VDP:VDP(21)=25:VDP(9)=VDP(9) OR 2\r25 POKE &HC000,1\r30 IF PEEK(&HC001)=0 THEN 30\r40 OUT &H99,13:OUT &H99,&H8E:OUT &H99,0:OUT &H99,&H7E:OUT &H98,&HA5:OUT &H98,&H5A\r50 POKE &HC000,2:GOTO 10\rRUN\r');
  for(const mode of [5,6,7,8]) {
    ...
    assert.deepEqual(values,[0xa5,0x5a,0,0],'SP3 requires contiguous CPU VRAM addresses in every screen mode');
  }
