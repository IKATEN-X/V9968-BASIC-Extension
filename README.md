...
**V9968が内蔵VDPとして構成された環境専用**です。外付けV9968カートリッジは対応対象に含みません。
開発中のプロジェクトです。現在はV9968版openMSXで検証しており、実機での動作は未検証です。

ROMを挿して通常起動すると、BASICの起動メッセージに続いて `V9968 BASIC Extension 0.1` と表示されます。デモなどを自動実行する場合は省略します。
ディスクから自動実行する場合は、最初にBASICのプロンプトへ戻った時点で表示されます。
...
- **フォントと素材作成**：8×8フォント、半角8×16・全角16×16の漢字を従来の `GRP:` 出力で使用。表示モードに依存しない4bppの素材描画。

標準BASICの変数・配列・式評価・ファイル操作と組み合わせて使えます。BASIC全体の画面・ページ制限を置き換えるものではなく、拡張機能には専用命令を使用します。

## デモ

回転COPY、半透明スプライト、32枚のスプライト波形、SCREEN 8のパレット、フォント、論理COPY、円・楕円、操作できる3Dワイヤーフレーム、シューティング、道路走行の試作などのBASICデモを収録しています。
...
| エミュレータ | openMSX 21.0のデータと、buppu3氏のV9968版Windowsバイナリ **`c620b69`（2026-09-23）** |
| 機種 | 内蔵VDPをV9968にしたFS-A1ST構成（R800）、最小MSX2+構成（Z80） |
| システムROM | 利用者自身で用意したMSXのBIOS・機種用ROM |

機種定義は新レジスタ定義の `V9968` を使用します。通常のV9958構成、旧定義の `V9968_OLD`、旧バイナリ `d884c4b` は対応対象外です。今後の別ビルドとの互換性は確認後に更新します。
...
| スプライトモード3 | SCREEN 1～6（フラットインターレース不可） |
| 拡張フォント | SCREEN 5、8×8／半角8×16／全角16×16。日本語は漢字ROMが必要（フラットインターレース不可） |
| パターン素材描画 | 表示モードとは別にSCREEN 5形式の4bpp画像を作成 |

- SCREEN 10～12のYJK描画、LFMCによるフォント直接転送、割り込みイベント用BASIC命令は未実装です。
- スプライトモード3のSCREEN 7以降は、VRAM配列切替の修正後の統合検証が未完了のため、引き続き無効にしています。
- 検証対象forkには、上位VRAMページの直接表示、SCREEN 6のモード3スプライトのX=256以降に制限があります。以前のSCREEN 8フラットインターレース表示の右端欠けは、更新版で解消を確認しました。
